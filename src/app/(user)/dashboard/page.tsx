...
const page = async () => {
    ...
    return(
        ...
        md:grid-cols-4 gap-4 mb-6">
            ...
            <>{
                userData?.map((metric) => <MetricCard key={metric.label} label={metric.label}
                value={metric.value}/>)
                }</>: null
            }
            </div>
            <div>
                {
                    heatMapData? <SubmissionsHeatMap data={heatMapData.data.map(item => ({ ...item, count: Number(item.count) }))} /> 
                    : null
                }
            </div>
