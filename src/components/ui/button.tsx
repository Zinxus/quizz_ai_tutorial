...
const neoClasses = "w-full rounded-full px-3.5 py-5 border-2 realtive z-10 text-lg font-bold hover:transform hover translate-y-[-2px] transition-tranfrom-duration-200"
  ...
  ["absolute", "h-14", "bottom-[7px]", "w-full", "border-2", "left-0", "z-0"],{
    variants: {
      variant: {
        ...
        neoSuccess: "border-green-600 bg-green-500",
        neoDanger: "border-red-600 bg-red-500",
      },
    },
    defaultVariants: {
      variant: "default",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const isNeo = variant === "neo" || variant === "neoOutline" || variant === "neoSuccess" || variant === "neoDanger";
    ...
    return (
      <div className={cn(isNeo && "relative w-full")}>
        <Comp
          ref={ref}
          className={cn(buttonVariants({ variant, size }), className, "relative z-10")}
          {...props}
        >
          <span className={cn(spanvariants({ variant }), borderRound, "absolute inset-0 z-0")} />
